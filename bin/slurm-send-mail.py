...
if __name__ == "__main__":
    ...
    try:
        ...
        if config.has_option(section, 'useSES'):
            useSES = config.getboolean(section, 'useSES')
            sesRegion = config.get(section, 'sesRegion')
            if config.has_option(section, 'sesRoleARN'):
                sesRoleARN = config.get(section, 'sesRoleARN')
            else:
                sesRoleARN = None
        else:
            useSES = False
            ...
            try:
                ...
                if state in ['Began', 'Ended', 'Failed']:
                    ...
                    if rtnCode == 0:
                        ...
                        if useSES:
                            from ses import SesSendMail
                            s = SesSendMail(sesRegion, role_arn = sesRoleARN)
                            subject = 'Job %s.%d: %s' % (cluster, jobId, state)
                            s.sendmail(emailFromUserAddress, userEmail, subject, body, body)
                        else:
                            # create HTML e-mail
